...
export default function ConsultingWebsite() {
  ...
  const sectionBackgrounds = {
    ...
    contact: svgToDataUri(`<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1600 700'><defs><linearGradient id='g' x1='0' y1='0' x2='1' y2='1'><stop stop-color='#0b3b72'/><stop offset='1' stop-color='#0e7490'/></linearGradient></defs><rect width='1600' height='700' fill='url(#g)'/><rect x='82' y='84' width='1436' height='536' rx='28' fill='#f0f9ff' opacity='.12'/><g font-family='monospace' font-size='23' fill='#67e8f9' opacity='.34'><text x='126' y='154'>fit_lm &lt;- lm(outcome ~ age + sex + treatment, data = trial_df)</text><text x='126' y='196'>fit_glm &lt;- glm(event ~ exposure + confounder, family = binomial, data = trial_df)</text><text x='126' y='238'>summary(fit_lm); confint(fit_glm); plot(fit_lm, which = c(1,2,5))</text><text x='126' y='298'>surv_obj &lt;- Surv(time_to_event, status)</text><text x='126' y='340'>km_fit &lt;- survfit(surv_obj ~ treatment_arm, data = cohort_df)</text><text x='126' y='382'>cox_fit &lt;- coxph(surv_obj ~ age + strata(site) + treatment_arm, data = cohort_df)</text><text x='126' y='442'>long_model &lt;- lmer(bp ~ week + treatment + (week | patient_id), data = long_df)</text><text x='126' y='484'>ts_fit &lt;- arima(ts(hospital_admissions, frequency = 12), order = c(1,1,1))</text><text x='126' y='526'>hypothesis_tests &lt;- list(t.test(group_a, group_b), chisq.test(table(exposure, outcome)))</text><text x='126' y='568'>anova(fit_lm); shapiro.test(residuals(fit_lm)); wilcox.test(marker ~ arm, data = trial_df)</text></g><line x1='106' y1='606' x2='1494' y2='606' stroke='#22d3ee' stroke-width='4' opacity='.24'/></svg>`)
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800">
      ...
      <section
        ...
        style={{
          backgroundImage: `linear-gradient(rgba(239, 246, 255, 0.7), rgba(236, 254, 255, 0.66)), ${sectionBackgrounds.contact}`,
          backgroundSize: "cover, cover",
          backgroundRepeat: "no-repeat, no-repeat",
          backgroundPosition: "center, center",
